...
public class MusicBarrierPartial : MonoBehaviour
{
    ...
    private void OnTriggerEnter(Collider other)
    {
        ...
        {
            Debug.Log($"[Barrera Parcial] ¡Música incorrecta! Necesitabas: {TrackNames[requiredTrack]} | Tenías: {TrackNames[active]}");
            other.GetComponent<PlayerHealth>()?.TakeDamage();
        }
        else
        {
            Debug.Log($"[Barrera Parcial] ¡Correcto! Pasaste con {TrackNames[active]}");
        }
    }
